...
public class Server
{
    ...
    public void Pokreni()
    {
        ...
        foreach (var iznaj in iznajmljivanja)
        {
            ...
            {
                Console.WriteLine($"Clan {iznaj.ClanID} kasni sa: {iznaj.Knjiga} ({iznaj.BrojPrimeraka}) od {iznaj.DatumVracanja.ToShortDateString()}");
            }
        }
    ...
    private void ObradiKlijentaTCP(TcpClient klijent, int id)
    {
        ...
        while (true)
        {
            ...
            else if (poruka.StartsWith("VRATI"))
            {
                ...
                iznajmljivanja.RemoveAll(i => i.ClanID == id && (i.Knjiga.Naslov + i.Knjiga.Autor) == kljuc);
